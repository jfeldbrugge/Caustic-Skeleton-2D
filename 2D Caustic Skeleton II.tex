...
\end{subfigure}
\caption{The distirubtion of the eigenvalue fields. \textit{Left:} the PDF of the initial density perturbation $\delta$ (black) and the eigenvalue fields $\mu_{i,t}$ (red and blue). \textit{Right:} the joint distribution of the eigenvalue fields.}\label{fig:eigen_stats}
\end{figure}
...
In tabel \ref{table:caustics}, we summarize the different elements of the caustic skeleton. For a complete description of caustic skeleton theory, we refer to the papers \cite{Hidding:2014, Feldbrugge:2018}.\\
...
\end{subfigure}
\caption{Elements of the caustic skeleton. \textit{From top to bottom:} the cusp, the swallowtail, the elliptic and the hyperbolic caustic. \textit{Left:} the caustic skeleton in Lagrangian space. \textit{Centre:} the Zel'dovich approximation. \textit{Right:} the corresponding $N$-body simulation.}\label{fig:caustics_Examples_Big}
\end{figure}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{The Zel'dovich approximation}\label{sec:Zeldovich}
In this paper we define the caustic skeleton using the Zel'dovich approximation \cite{Zeldovich:1970} and study the resulting structures using a dark matter $N$-body simulation. The Zel'dovich approximation is the first order approximation in Lagrangian fluid dynamics, for which the displacement field factorizes into a spatial and a temporal part
\begin{align}
\bm{s}_t(\bm{q}) = - b_+(t) \nabla_{\bm{q}} \Psi(\bm{q})\,.
\end{align}
...
\end{align}
defined in terms of the mean field $\bar{f}(\bm{q})$ and the kernel $K(\bm{q}_1,\bm{q}_2)$ \cite{adler:1981, bbks:1986}. The probability that the random field $f$ is included in a set of functions $\mathcal{S}$ is defined by the path integral
\begin{align}
...
\end{align}
with the magnitude $k = \|\bm{k}\|$. The powerspectrum determines the covariance matrix $M$ through these moments. Whereas is the variance of the random field is given by the moment $\sigma_0^2$, \textit{i.e.},
\begin{align}
...
\end{align}
with $\bm{k}=(k_1,k_2)$ and $\bm{p}=(p_1,p_2)$. By statistical isotropy, we obtain the variance of the first order partial derivative
\begin{align}
...
&=
- 4 (T_{111} T_{122}^3 + T_{112}^3 T_{222}) + 3 T_{112}^2 T_{122}^2 +  6 T_{111} T_{112} T_{122} T_{222} - T_{111}^2 T_{222}^2\,,
\end{align}
...
\begin{align}
\delta = \frac{1}{2}(-T_{112}^2 + T_{111} T_{122} - T_{122}^2 + T_{112} T_{222}).
\end{align} 
...
To study the interplay between the voids, walls, filaments and clusters in the cosmic web and study the connectivity, it is important to extend constraint theory of the caustic skeleton to include mulitple constraints with specified orientations. In this section, we show how to implement the generation realizations with non-linear composite constraints.
...
\item
Evaluate all derivatives of the realization to the relevant order. Generally, some derivatives were obtained while sampling the constraint manifold $\mathcal{M}_\mathcal{C}$. Others do not directly influence the caustic, and are sampled with the realization. For the cusp, we evaluate the partial derivatives 
\begin{align}
...
\item 
Under a rotation of the coordinate system by an angle $\theta$, the $n$-th order derivatives rotate into each other (see appendix \ref{ap:rotations}). Use these transformation rules to obtain a represenative sample of the reqiured partial derivatives for the caustic with the desired orentation.
\item 
Finally, use the Hoffman Ribak algorithm to obtain a realization satisfying the caustic condition with the desired orientation.
\end{enumerate}

See figure \ref{fig:rotation} for an illustration of the cusp caustic with three orientations. In the left panel, the cusp caustic passes vertically through the centre of the box. In the central and right panels, we evaluate the second, third, and fourth order derivatives and generate random fields for which the cusp curve passes diagonally and horizontally through the the center of the box. For these three realizations applied the Hoffman Ribak algorithm with a single unconstrained random field. As a consequence, we see that the structure of the initial conditions away from the centre $(100,100)$ remain unchanged. Using constrained Gaussian random field theory, we can locally tweak the random field to satisfy the required constraints.
...
\end{align}
The critical point is either a maximum, minimum -- corresponding to the Morse point $A_3^+$ -- or a saddle point -- corresponding to the Morse point $A_3^-$ -- determined by the sign of the determinant of the Hessian $\mathcal{H}\lambda_1$. In the eigenframe the Hessian takes the form
\begin{align}
\mathcal{H}\lambda_1 = 
\begin{pmatrix} 
...
\end{pmatrix}\,.
\end{align}
In figure \ref{fig:composite}, the lower, the upper, and the right critical points are $A_3^+$ points. The left critical point is a $A_3^-$ point. In the right panel we can clearly distinguish the different nature of the $A_3^+$ and $A_3^-$ points in the Zel'dovich approximation. The $A_3^+$ points correspond to the creation of a multi-stream reagion, while the $A_3^-$ points correspond to the merger of two multi-stream regions.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{The caustic skeleton in Eulerian space}
To develop a visual appreciation of the roles of the different caustics in the large scale structure and the accuracy of the caustic skeleton of the Zel'dovich approximation, we construct a realization of an unconstrained Gaussian random field for the gravitational potential $\phi_0$ and evolve it with the Zel'dovich approximation and a dark matter $N$-body simulation (see figure \ref{fig:Eulerian}). Observe the geometry of the first eigenvalue and eigenvector field $\lambda_1$ and $\bm{v}_1$ and the corresponding caustic skeleton (see the upper left panel). At late times, the cusp curves form an interconnected network tracing multi-stream regions bounded by the fold curves. The blue regions bounded by the fold curves are the progentors of the voids. At large scales, the eigenvalue field closely resembles the initial density perturbation $\delta_0 \propto \nabla^2\phi_0$ (see the upper right panel). At large scales, the caustic skeleton follows geometry of the superlevel sets of the density perturbation. At small scales, we observe qualitative differences. The Zel'dovich approximation corresponding to this setup is far outside the range of applicability, as the Zel'dovich approximation linearly extrapolates the motion for the mass elements and ignored the further gravitational interactions (see the lower left panel). However, even though the Zel'dovich approximation is no-longer an accurate model for density field, it accurately predicts which mass elements undergo shell-crossing and forming caustics at late times (see the lower right panel). The fold curves provide an reasonable approximation of the boundaries of the multi-stream regions. The cusp curves neatly bisect the multi-stream regions and form knots in the clusters of the cosmic web. Note that a cluster rarely consists of a single cluster caustics. The caustics are correlated and together form a more intricate emergent knot. The filaments evolve more linearly and can be associated to an unique cusp curve.
\begin{figure}
\centering
\begin{subfigure}[b]{0.49\textwidth}
...
The multi-stream regions of the cosmic web forms gradually and its evolution is neatly captured by the caustic skeleton (see figure \ref{fig:Eulerian_Evolution}). At early times, the dark-matter sheet consisted of a single single-stream region. As time evolved, we observe the formation of several Zel'dovich pancakes corresponding to the maxima of the eigenvalue field $\lambda_1$ (see the upper left panel). As time proceeds, these multi-stream regions grow and connect at the saddle points of the eigenvalue fields (see the upper right and lower panels). Note that in the evolution of the cosmic web the filaments move while the large voids expand and the smaller voids contract. In this process different filaments can merge to form a more massive structure.
