...
While there have been attempts to augment the linear Gaussian field formalism by invoking a (first order) model for the displacement of mass elements in order to follow these along their path backward in time\citep{}. Because of the restricted validity to regions that are not (yet) multistream, it cannot be applied for our purpose of modeling the mass distribution around the intrinsically multistream caustic structures. Given the fact that the majority of matter, halos and galaxies reside in filaments \citep{Cautun:2014,Ganeshaiah:2019}, more sophisticated higher-order schemes will be necessary. The Least Action Principle algorithm introduced by \cite{peebles1983} enables us to follow the orbits of galaxies back in time. More sophisticated higher-order Lagrangian formalisms have been proposed by \cite{Mohayaee:2006} and \cite{baojili2020}. \cite{Mohayaee:2006} used the Monge-Amp\`ere-Kantorovitch algorithm to follow the orbits of objects back in time, culminating in a compelling reconstruction of the local velocity field corresponding to the 2MRS survey. 
...
these are still restricted to a regime in which mass elements have not yet passed through multistream regions. W it cannot be applied for our purpose of modeling the mass distribution around the intrinsically multistream caustic structures. 
