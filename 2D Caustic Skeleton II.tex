...
In figure \ref{fig:Eulerian}, we see how the different elements of the caustic skelelton come together to model the cosmic web. Observe that the different singularities do not occur in isolation. They are correlated and form more intricate emergent structures. This explains why a knot might connect more than three filaments. Moreover note that while the density estimate of the Zel'dovich approximation might for late times be outside its range of applicability, the corresponding caustic skeleton is predictive long after this time.
...
\end{subfigure}
\caption{The caustic skeleton. \textit{Upper left:} the initial density perturbations with the caustic skeleton. \textit{Upper right:}  the initial eigenvalue field $\lambda_1$ with the caustic skeleton. \textit{Lower left:} the Zel'dovich approximation with the caustic skeleton. \textit{Lower right:} the $N$-body simulation with the caustic skeleton.}\label{fig:Eulerian}
\end{figure}

In figure \ref{fig:Eulerian_Evolution}, we plot the evolution of the caustic skeleton in an $N$-body simulation.
...
\item develop a fast routine to develop constrained fields with multiple constraints,
