...
For the umbilic points, corresponding to the first two eigenvalue fields, we know that in the eigenframe for which $\bm{v}_1=(1,0,0)$, $\bm{v}_2=(0,1,0)$, $\bm{v}_3=(0,0,1)$, the deformation tensor is of the form
\begin{align}
\bm{\psi} = \begin{pmatrix} \alpha & 0 & 0 \\ 0 & \alpha & 0 \\ 0 & 0 & \beta\end{pmatrix},
\end{align}
with the eigenvalues $\alpha = 1/b_+$ and $\beta < \alpha$. To distinguish between the elliptic and hyperbolic umbilics, we evaluate the sign of the determinant 
\begin{align}
&\det (\mathcal{H}_{1,2}\left[\det (I- b_+ \bm{\psi})\right]) \\
&= 
b_p^4 (b_p \beta - 1)^2
\left[
 6 T_{111} T_{112} T_{122} T_{222} + 3 T_{112}^2 T_{122}^2  - 4 (T_{111} T_{122}^3 + T_{112}^3 T_{222})  - T_{111}^2 T_{222}^2\right]
\end{align}
where $\mathcal{H}_{i,j}$ is the Hessian in the $\bm{v}_i$, $\bm{v}_j$ directions. When this determinant is positive/negative, we have a elliptic/hyperbolic umbilic caustic. When the determinant vanishes, we have an butterfly caustic. When the umbilic caustic is associated to the second and third eigenvalue fields, the condition follows mutandis mutatis.
\end{document}
